/*
 * Eaton XComfort Dimmer
 *
 * Hubitat connecting to the Eaton XComfort dimmer using HTTP
 *
...
metadata {
    definition(name: "Eaton XComfort Dimmer", namespace: "community", author: "Supun Vidana Pathiranage", importUrl: "https://raw.githubusercontent.com/vpsupun/hubitat-eaton-xcomfort/master/EatonXComfort.groovy") {
        capability "Actuator"
        capability "Switch"
...
void setLevel(level, duration=0) {
    triggerSetLevel(level as Integer, duration as Integer)
}

void triggerSetLevel(Integer value, Integer duration) {
    if (logEnable) log.debug "Setting the level to ${value} in the device, [${settings.dev}] on the zone, [${settings.zone}]"
    List params = ["${state.zoneId}", "${state.deviceId}", "${value}"]
...
void getStatus() {
    if (state.zoneId != null || state.deviceId != null) {
        ...
        if (devices != []) {
            def filtered = devices.flatten().find { it.id == state.deviceId }
            if (filtered != null) {
...
void triggerOn() {
    ...
    try {
        httpPostJson(httpParams) { resp ->
            if (resp.success) {
                sendEvent(name: "switch", value: "on", isStateChange: true)
                sendEvent(name: "switchLevel", value: 100, isStateChange: true)
            }
            if (logEnable)
                if (resp.data) log.debug "${resp.data}"
        }
...
void triggerOff() {
    ...
    try {
        httpPostJson(httpParams) { resp ->
            if (resp.success) {
                sendEvent(name: "switch", value: "off", isStateChange: true)
                sendEvent(name: "switchLevel", value: 0, isStateChange: true)
            }
            if (logEnable)
                if (resp.data) log.debug "${resp.data}"
        }
        ...
        if (devices != []) {
            ...
            if (filtered != []) {
                if (filtered.type == "DimActuator") {
                    ...
                    if (logEnable) log.debug "Device is not a dim actuator (type : ${filtered.type}"
